...
2. The orchestrator — a pure coordinator (passes paths, reads the spec header, emits checkpoints) and the role roster's maintainer (glossary: `role roster`): creates `.claude/tmp/<slug>/roster.md` at the first dispatch, updates a row's state at every state change, and applies the role's output-contract parse rule (glossary: `output contract`) at each return point. Relayed numeric claims (counters, sizes, line counts) enter task prompts and summaries only as copy-paste from a fresh run, with the run command quoted in the brief — not from memory or arithmetic. Before launching audit — a control re-run of the spec's own counters (header, registry tables, summary records): a discrepancy — return to the Specifier.
3. **Specifier** — a subagent (fresh for complex changes). Mandate — [`assets/specifier-charter.md`](assets/specifier-charter.md). Produces the 5-strata spec from the charter + the codebase. Self-change cycle: the finalized spec carries the L1–L5 × evidence invariant table (PCC; legitimacy-invariants). Dispatch/return — the two-branch completion rule (glossary: `dispatch-completion`; canon — [`../../norms/dispatch-completion.md`](../../norms/dispatch-completion.md)): a spec at FINAL (the exit self-check green) completes the launch; a launch ending before a parsable FINAL state is completed by the same agent within the same frame.
4. Checkpoint 1 → the authoring point (halt) → resolution → (Checkpoint 2) → spec finalized → passed to `audit-change`. A halt is a wait, not a completion: work continuing after a checkpoint resolution or a frontier answer continues in the same Specifier launch; a post-FINAL return — a control re-run counter mismatch, audit `blocked` findings, a MINOR fix-now — is a fresh Specifier dispatch with a fix-pass brief (spec + charter + findings + round-ledger with class history + RCA outcomes + disposition records); the checkpoint canon applies to the fix-pass.
