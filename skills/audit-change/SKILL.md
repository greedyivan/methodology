...
2. **File exchange.** Artifacts — files in `.claude/tmp/audit-change-<slug>/`. The orchestrator passes paths, reads only headers (counters). The orchestrator cross-checks relayed numeric claims (counters, sizes, line counts) by a run before retransmission into task-prompts and summaries.
