...
The deliverable — the executed S4 units + the cycle's impl-ledger (RL records
for every realize decision). The parse rule, applied by the orchestrator at
the return point: the launch is completed when the orchestrator confirms
Tier-1 green — the spec's fitness gates green on the full index surface, the
impl-ledger carrying the units' records, and the cycle-zone deliverables
present — and marks the roster row completed; a confirmed Tier-1 red or a due
L1 fix opens a fresh realizer (not a resume); halting-table stops inside the
mandate return to the same realizer (a halt is a wait, not a completion —
glossary: `dispatch-completion`).
