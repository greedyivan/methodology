...
2. **File exchange.** Artifacts — files in `.claude/tmp/audit-change-<slug>/`. The orchestrator passes paths, reads only headers (counters). Relayed numeric claims (counters, sizes, line counts) enter task-prompts and summaries only as copy-paste from a fresh run, with the run command quoted in the brief — not from memory or arithmetic.
