...
- **A gate is entered with a run-pin :** a gate/check/pattern enters the spec only with the author's run at the moment of entry — the fact "catches X / does not catch Y" next to the formulation; a declarative gate without a run reaches the audit as false-security.
  ...
  **Adversary-independence of negative witnesses :** a birth-fence negative is authored from a source independent of the pattern author's model where one exists — corpus facts (live tag spellings, baseline file shapes, live headers) are the preferred source; a negative crafted solely from the pattern's own vocabulary is a correlated witness and closes no class beyond its own bytes. **Per-escape-class witnessing :** every mechanical check enumerates the fixed escape-class list — inflection/verbal paradigm; casing; prefix/adjoined forms; carrier domain (fenced block, inline code, prose, header, table); line shape (wrap/continuation, compound clauses); multi-occurrence per line; non-canonical mention/ID forms; engine semantics — and each class is either legged with a run-born witness or bounded with a recorded ground; a check that legs one class while silently implying closure of the rest is a false-security defect. **Degenerate-positive ban :** a positive witness whose green output is byte-identical to the command's failure mode (a diff against a missing path, a count over an absent stream) is not a witness — the fence distinguishes success from not-found (exit-code legs, both-side counts).
