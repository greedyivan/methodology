...
2. The orchestrator — a pure coordinator (paths, headers) and the role roster's maintainer (glossary: `role roster`): creates `.claude/tmp/<slug>/roster.md` at the first dispatch, updates a row's state at every state change, and applies each role's output-contract parse rule (glossary: `output contract`) at its return point. **Confirms Tier-1 green** (the gates) before starting the Verifier.
3. **The Verifier** — a fresh subagent (not the realizer). Mandate — [`assets/verifier-charter.md`](assets/verifier-charter.md). Dispatch/return — the two-branch completion rule (glossary: `dispatch-completion`; canon — [`../../norms/dispatch-completion.md`](../../norms/dispatch-completion.md)): the verdict parsing into the header format is the launch's completion moment (never re-entered); a launch ending without a parsable verdict is completed by the same agent within the same frame. Checks **Tier-2 semantic only** (mechanical is already Tier-1); delivers an adjudication per discrepancy.
...
5. **read-reinspect (closing L1):** every applied L1 fix (fix-realization) is closed by a read-reinspect — a fresh reader (a subagent ≠ realizer and ≠ the Verifier who delivered the adjudication; the ≠-constraints evaluate over the whole role roster — glossary: `role roster`) re-reads the applied fixes + their immediate consumption-sites; the outcome (confirmation/discrepancy) is recorded in the verdict artifact. The reader's return is governed by the two-branch completion rule (glossary: `dispatch-completion`): the report parsing into the reader's contract completes the launch; a launch ending without it is completed by the same agent. Mandatory at ≥1 applied L1 fix of any application loop — including the orchestrator's fixes in a run combining the Tier-1 loop and the Tier-2 checkpoint; a cycle without L1 fixes generates no step. Not a second verify run: a scoped re-check of the changed, within a single checkpoint. Differentiation from the audit stage's `re-inspection-scope` — glossary.
